...
from database.sql.models import Belge, DenetimIzi, Kullanici
...
def register(req: RegisterRequest, db: Session = Depends(get_db)):
    try:
        yeni_kullanici = Kullanici(
            tam_ad=req.tam_ad, eposta=req.eposta, sifre_karma=hash_password(req.sifre)
        )
        ...
        db.rollback()
        raise HTTPException(
            status_code=400, detail="Bu e-posta adresi zaten kullanılıyor."
        )
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=500, detail="Sunucu hatası: Can't register user"
        )
...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    if not is_db_ready():
        raise HTTPException(
            status_code=503,
            detail="Sistem henüz hazırlanıyor, lütfen birkaç saniye sonra tekrar deneyin.",
        )
    ...
    if not kullanici:
        ...
        log = DenetimIzi(
            islem_turu="LOGIN_FAILED_NOTFOUND",
            eski_deger={"eposta": req.eposta},
            ip_adresi="N/A",
        )
        db.add(log)
        db.commit()
    ...
    if kullanici.aktif_mi is False or kullanici.aktif_mi == 0:
        log = DenetimIzi(
            kullanici_kimlik=kullanici.kimlik,
            islem_turu="LOGIN_FAILED_SUSPENDED",
            ip_adresi="N/A",
        )
        db.add(log)
        db.commit()
    ...
    # 3. Sifre yanlis
    if not kullanici.sifre_karma or not verify_password(
        req.sifre, kullanici.sifre_karma
    ):
        log = DenetimIzi(
            kullanici_kimlik=kullanici.kimlik,
            islem_turu="LOGIN_FAILED_PASSWORD",
            ip_adresi="N/A",
        )
        db.add(log)
        db.commit()
    ...
    log = DenetimIzi(
        kullanici_kimlik=kullanici.kimlik, islem_turu="LOGIN_SUCCESS", ip_adresi="N/A"
    )
    db.add(log)
    db.commit()
...
def get_users(db: Session = Depends(get_db)):
    ...
    session_rows = (
        db.query(
            DenetimIzi.kullanici_kimlik, func.count(DenetimIzi.kimlik).label("cnt")
        )
        .filter(
    ...
    token_rows = (
        db.query(
            ...
            func.coalesce(func.sum(ApiLog.toplam_token), 0).label("tok"),
        )
    ...
    return [
        {
            ...
            "email": u.eposta,
            "role": (
                "Sistem Yöneticisi" if u.super_kullanici_mi else "Standart Kullanıcı"
            ),
...
def update_user_status(
    user_id: str, req: StatusUpdateRequest, db: Session = Depends(get_db)
):
    ...
    if user.aktif_mi != yeni_aktif_durum:
        raise HTTPException(
            status_code=500, detail="Durum kaydedilemedi, tekrar deneyin."
        )
...
def update_user_role(
    user_id: str, req: RoleUpdateRequest, db: Session = Depends(get_db)
):
    user = db.query(Kullanici).filter(Kullanici.kimlik == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Kullanıcı bulunamadı")
    ...
    return {
        ...
        "user_id": user_id,
        "yeni_rol": (
            "Sistem Yöneticisi" if user.super_kullanici_mi else "Standart Kullanıcı"
        ),
    }
...
def update_user_meta(
    user_id: str, meta: MetaGuncelleRequest, db: Session = Depends(get_db)
):
    user = db.query(Kullanici).filter(Kullanici.kimlik == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Kullanıcı bulunamadı")
...
def update_user_profile(
    user_id: str, req: ProfileUpdateRequest, db: Session = Depends(get_db)
):
...
def get_user_dashboard(user_id: str, db: Session = Depends(get_db)):
        ...
        recent_docs.append(
            {
                ...
                "date": (
                    b.olusturulma_tarihi.split("T")[0]
                    if b.olusturulma_tarihi
                    else "Yakın Zamanda"
                ),
            }
        )
    ...
    egitimler = meta.get(
        ...
        [
            {
                "isim": "Temel YZ Kullanım Eğitimi",
                "durum": "Tamamlandı",
                "renk": "emerald",
            },
            {
                "isim": "İleri Seviye Prompt Mühendisliği",
                "durum": "%60 Devam Ediyor",
                "renk": "amber",
            },
        ],
    )
    ...
    return {"egitimler": egitimler, "belgeler": recent_docs, "talepler": talepler}
...
def update_user_file_access(
    user_id: str, req: FileAccessUpdate, db: Session = Depends(get_db)
):
    user = db.query(Kullanici).filter(Kullanici.kimlik == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Kullanıcı bulunamadı")
    ...
    if req.has_access:
        ...
        if belge.havuz_turu == "sistem" and not izinliler:
            tum_kullanicilar = (
                db.query(Kullanici.kimlik)
                .filter(Kullanici.super_kullanici_mi == False)
                .all()
            )
...
def get_permissions_context(user_id: str, db: Session = Depends(get_db)):
    ...
    system_tabs = [
        {
            "key": "ui_file_processing",
            "label": "Dosya İşleme",
            "desc": "Dosya yükleme, işleme ve arşiv yönetimi.",
        },
        {
            "key": "ui_database",
            "label": "Veritabanı",
            "desc": "Sistemdeki dosyalara göz atma ve silme.",
        },
        {
            "key": "ui_ai_orchestrator",
            "label": "Yapay Zeka Merkezi",
            "desc": "YZ modelleri, agentlar ve chatbot etkileşimi.",
        },
        {
            "key": "ui_metrics",
            "label": "Sistem Metrikleri",
            "desc": "Gelişmiş analitik grafikleri ve izleme logları.",
        },
        {
            "key": "ui_auth",
            "label": "Kullanıcı ve Rol Yönetimi",
            "desc": "Kullanıcı, rol ve yetki yönetimi (yönetici).",
        },
    ]
...
def get_live_dashboard(db: Session = Depends(get_db)):
    ...
    users = db.query(Kullanici).all()
    user_map = {
        u.kimlik: {"name": u.tam_ad, "status": "Aktif" if u.aktif_mi else "Pasif"}
        for u in users
    }
    ...
    denetim_logs = (
        db.query(DenetimIzi)
        .order_by(DenetimIzi.olusturulma_tarihi.desc())
        .limit(1500)
        .all()
    )
    ...
    with get_session() as logs_db:
        ...
        # ⚡ Bolt: Perform aggregation in the database instead of loading unbounded arrays into memory.
        # This fixes a memory bloat issue where thousands of logs for the current day were fetched and looped over.
        api_log_stats = logs_db.execute(
            select(
                ApiLog.kullanici_kimlik,
                func.count().label("req_count"),
                func.coalesce(func.sum(ApiLog.toplam_token), 0).label("token_sum"),
            )
            .where(ApiLog.olusturulma_tarihi.like(f"{today}%"))
            .group_by(ApiLog.kullanici_kimlik)
        ).all()

        for row in api_log_stats:
            uid = row.kullanici_kimlik
            total_signals += row.req_count
            if uid in stats_map:
                stats_map[uid]["api_requests"] += row.req_count
                stats_map[uid]["total_tokens"] += int(row.token_sum)

    for d in denetim_logs:
        uid = d.kullanici_kimlik
        if d.islem_turu.startswith("LOGIN_FAILED") and d.olusturulma_tarihi.startswith(
            today
        ):
            failed_logins += 1
            ...
            recent_events.append(
                {
                    ...
                    "color": (
                        "red"
                        if "FAILED" in d.islem_turu
                        else "emerald" if "SUCCESS" in d.islem_turu else "blue"
                    ),
                }
            )
        ...
        if uid in stats_map:
            ...
            if (
                d.islem_turu == "LOGIN_SUCCESS"
                and stats_map[uid]["_last_login"] is None
            ):
