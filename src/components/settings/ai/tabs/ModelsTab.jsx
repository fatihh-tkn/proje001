...
import {
    Key, Plus, Eye, EyeOff, CheckCircle2,
...
function ModelCard({ model, index, alias, onAliasChange, onDelete }) {
    ...
    useEffect(() => {
        let isMounted = true;
        // Sessizce arka planda bağlantıyı kontrol et
        const checkStatus = async () => {
            try {
                ...
                }, 10000); // Canlı sorgu için 10sn timeout limit
                    ...
                    <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="min-w-0 flex-1 flex flex-col pt-0.5">
                            ...
                            <div className="flex items-center justify-between gap-2 max-w-full">
                                ...
                                {/* Status Badge (Artık absolute değil, düzgün hizalı) */}
                                <div className="flex shrink-0">
                                    {status === 'checking' && (
                                        <span className="flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-full text-slate-400 bg-slate-500/10 border border-slate-500/30">
                                            <RefreshCw size={8} className="animate-spin" /> BAĞLANIYOR
                                        </span>
                                    )}
                                    {status === 'active' && (
                                        <span className="flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-full text-emerald-500 bg-emerald-500/10 border border-emerald-500/30">
                                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block" /> AKTİF
                                        </span>
                                    )}
                                    {status === 'inactive' && (
                                        <span className="flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 rounded-full text-red-500 bg-red-500/10 border border-red-500/30">
                                            <X size={10} className="text-red-500" /> PASİF
                                        </span>
                                    )}
                                </div>
                            </div>

                            {/* Detay bilgileri (Sistem modeli + API Key) */}
